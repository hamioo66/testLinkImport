...
def readExcel(file_path):
    ...
    try:
        ...
        logger.error(u'路径不在或者excel不正确 : ' + str(error))
        return error
    else:
...
def excute_creat_testcase(test_project_id, test_father_id, test_file_name):
    ...
    logger.info(u"本次操作共提交 {} 条数据，成功导入 {} 条，失败 {} 条".format(count_success + count_fail, count_success, count_fail))

if __name__ == "__main__":
    url = "http://10.33.94.71/testlink/lib/api/xmlrpc/v1/xmlrpc.php"  # 替换为testlink对应URL
    # http://127.0.0.1/testlink/lib/api/xmlrpc/v1/xmlrpc.php

    #key = "6e91d45cb7def22c71540526d7cba739"  # 这个key是错误的key，登陆testlink后点击上方个人账号进入个人中心，新页面点击 '生成新的秘钥'获取
    key = "258da711c0de7c0f0a46ffe96315195e"
    file_name = "testCase_Example.xlsx"
    project_id = "10"  # 可以通过 print(get_projects_info())获取
    father_id = "27"  # 鼠标选中想要上传用例的用例集，点击右键获取父节点ID
    tlc = testlink.TestlinkAPIClient(url, key)
    print(u"项目信息: ", get_projects_info())
